...
class TrainerBase:
    ...
    def run(self):
        ...
        self.args.logger.info(
            "  Fraction of trainable parameters = %s",
            str(round(num_trainable_params / num_params * 100, 4)),
        )
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                if (step + 1) % self.args.grad_acc_steps == 0:
                    ...
                    if (
                        ...
                        reported_loss = self.report_loss_dict(
                            acc_loss_dict, self.args.logging_steps
                        )
                        self.args.logger.info(
        ...
        if (idx + 1) % self.args.save_epochs != 0:
            ...
            self.save(output_dir, global_step, idx + 1, None, None)
            self.save(
                last_output_dir, global_step, self.args.num_train_epochs, None, None
            )
...
def get_logits_from_lm(lm, inputs, control_ids):
    if control_ids is not None:
        ...
        past = None
    print("Debugging type:", type(past), type(inputs))
...
class PrefixTrainer(TrainerBase):
    ...
    def step(self, batch):
        ...
        lm_loss = token_weighted_loss(
            "cross_entropy", correct_logits, shift_inputs, shift_weights
        )
        ...
        if self.args.contrastive_loss_ratio != 0 or self.args.kl_loss_ratio != 0:
            ...
            if self.args.contrastive_loss_ratio != 0:
                ...
                contrastive_labels = torch.zeros(
                    shift_inputs.shape, dtype=torch.int64
